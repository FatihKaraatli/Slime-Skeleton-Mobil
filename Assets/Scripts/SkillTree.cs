...
public class SkillTree : MonoBehaviour
{
    ...
    public Transform closeButtonNewPosition;
    public Vector3 closeButtonFirstPosition;
    ...
    public void Start()
    {
        ...
        closeButtonFirstPosition = closeButton.transform.position;
    }
    ...
    void Update()
    {
        if (boardCreateControl)
        {
            if (isOpening)
            {
                ...
                board.transform.localScale = Vector3.Lerp(Vector3.zero, new Vector3(0.5f,0.5f,1), boardCreateTimer.NormalizedTime);
            ...
            else
            {
                boardCreateTimer.UpdateTimer();
                var rot = board.transform.rotation;
                board.transform.localScale = Vector3.Lerp(new Vector3(0.5f, 0.5f, 1), Vector3.zero, boardCreateTimer.NormalizedTime);
                board.transform.Rotate(0, 0, rotSpeed * Time.deltaTime);
                if (boardCreateTimer.Done())
                {
        ...
        if (skillsPressMovementControl && upgradeSceneControl)
        {
            ...
            {
                skills[i].transform.position = Vector3.Lerp(skillsFirstPosAndScales[i].position, skillsSmallPosition.position + new Vector3(1.85f * i, 0, 0), skillsPressMovementTimer.NormalizedTime);
    ...
    public void UpgradeScreen(int index)
    {
        ...
        closeButton.transform.position = closeButtonNewPosition.position;
    }
