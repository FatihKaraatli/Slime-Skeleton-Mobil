...
public class PlayerAttack : MonoBehaviour
{
    ...
    {
        if (punchTimer.Done() && !playerMovement.jump && playerMovement.horizontalMove == 0)
        {
            Punch();
        }
    }
    ...
    {
        if (defenceTimer.Done() && !playerMovement.jump && playerMovement.horizontalMove == 0)
        {
            Defence();
        }
    }
    ...
    {
        if (!dashControl && playerMovement.horizontalMove != 0)
        {
            RaycastHit2D[] hit2 = Physics2D.RaycastAll(transform.position, transform.right * transform.localScale.x, 25f, layer);
            {
                Dash(hit2);
    ...
    public void SlimeBulletButton()
    {
        if (fireBulletDestinationTimer.Done())
        {
            FireBall();
        }
    }
