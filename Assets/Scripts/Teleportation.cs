...
public class Teleportation : MonoBehaviour
{
    ...
    public Transform secondFloorCamPos;
    public Transform thirdFloorCamPos;
    ...
    void Update()
    {
        ...
        if (halfwayControl)
        {
            ...
            if (halfWayDestinationTimer.Done())
            {
                ...
                if (this.gameObject.name.Contains("First"))
                {
                    camHolder.transform.position = new Vector3(camHolder.transform.position.x, secondFloorCamPos.position.y, camHolder.transform.position.z);
                }
                else
                {
                    camHolder.transform.position = new Vector3(camHolder.transform.position.x, thirdFloorCamPos.position.y, camHolder.transform.position.z);
                }
                halfWayDestinationTimer.ResetTimer();
